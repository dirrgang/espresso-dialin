...
`DoseRecommendation` records the model identifier/version, context, target and recommended duration, estimated rate, expected output, exact source observation IDs, observation count, and latest included sequence. Its local ID is a deterministic hash of the model, target context, and history. `created_at` accepts a timezone-aware timestamp; it is `None` for historical reconstruction because no real pre-shot creation time exists. Stable dict/JSON serialization is provided for later SQLite persistence.
...
For **current** historical evaluation, use the corrected dataset:

```powershell
.venv\Scripts\python.exe -c "from pathlib import Path; from espresso_dialin.historical import load_shots, rolling_dose_report; print(rolling_dose_report(load_shots(Path('data/historical_shots_corrected.csv'))))"
```
...
### Archived staging-based benchmark

The first implementation run used `data/historical_shots_staging.csv` before the corrected dataset and bean identities became authoritative. It produced the following **archival snapshot**, which must not be used as the current benchmark without regeneration:
...
Those values remain useful for provenance and regression comparison, but the corrected CSV fills several previously missing settings/durations and identifies the bean blocks as Café Intención Espresso Intensivo and REWE Bio Espresso ganze Bohnen 1000 g. Eligibility and rolling sample counts may therefore change.

Even after regeneration, historical limitations remain: operator-adapted observational data, changing bean age and grinder state, incomplete session/timestamp information, and no randomized duration interventions. Historical metrics test rate/output prediction, not closed-loop convergence, coffee saved, or shots-to-target.

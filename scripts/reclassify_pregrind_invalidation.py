...
EXPECTED_RESOLUTION_UPDATE_TRIGGER = """CREATE TRIGGER resolution_update
...
def _verified_trigger_sql(db: sqlite3.Connection) -> str:
    rows = db.execute(
        "SELECT sql FROM sqlite_master WHERE type='trigger' AND name='resolution_update' "
        "AND tbl_name='shot_resolutions'"
    ).fetchall()
    if len(rows) != 1 or rows[0][0] is None:
        raise ValueError("expected immutable resolution_update trigger is missing")
    actual = str(rows[0][0])
    if _normalize_sql(actual) != _normalize_sql(EXPECTED_RESOLUTION_UPDATE_TRIGGER):
        raise ValueError("resolution_update trigger definition is not the expected schema-v2 guard")
    return actual
...
def _validate_target(db: sqlite3.Connection, row: sqlite3.Row) -> None:
    ...
    if version != 2:
        raise ValueError(f"repair requires schema version 2, found {version}")
...
def apply_repair(
    ...
    *,
    ...
    try:
        ...
        row = _target(db, session_id, shot_id)
        _validate_target(db, row)
        trigger_sql = _verified_trigger_sql(db)
...
def inspect_corrected(database: Path, session_id: str, shot_id: str) -> sqlite3.Row:
    """Read the repaired row and verify the immutable trigger remains present."""
    with sqlite3.connect(_read_only_uri(database), uri=True) as db:
        db.row_factory = sqlite3.Row
        row = _target(db, session_id, shot_id)

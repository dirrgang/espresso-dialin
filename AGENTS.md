...
- Python dependencies and Python-tool configuration belong in `pyproject.toml`; repository runtime/tool bootstrap belongs in `mise.toml`; resolved Python dependencies belong in `uv.lock`.
...
uv run --locked --extra dev pytest --cov=espresso_dialin --cov-report=term-missing
```
...
`prek` executes the existing `.pre-commit-config.yaml`. Do not weaken a quality gate merely to make a change pass; either fix the issue or document why the rule is inappropriate and adjust the configuration deliberately.
